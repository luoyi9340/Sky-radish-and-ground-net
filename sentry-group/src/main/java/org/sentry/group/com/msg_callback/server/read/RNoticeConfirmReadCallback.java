 ...
 *
 */
public class RNoticeConfirmReadCallback extends AServerMsgCallback<RNoticeConfirmMsg> implements IMsgReadCallback<RNoticeConfirmMsg>{
	...
	protected void callbackHandly(ChannelHandlerContext ctx, RNoticeConfirmMsg msg) {
		...
		if (!App.ME.getRule().equals(RuleEnums.Leader)) {
			...
			log.info("[RNoticeConfirmReadCallback] 收到RNoticeConfirmMsg消息，但本节点不是主节点，不予受理. resolutionUniquenessId:" + msg.resolutionUniquenessId());
			return;
		}
		...
		if (resolution == null) {
			...
			if (factory == null) {
				...
				log.info("[RNoticeConfirmReadCallback] 收到RNoticeConfirmMsg消息，但不包含此类型的决议，不予受理. resolutionUniquenessId:" + msg.resolutionUniquenessId());
				return;
			}
			...
			log.info("[RNoticeConfirmReadCallback] 收到RNoticeConfirmMsg消息，主节点已受理，决议开启. resolutionUniquenessId:" + msg.resolutionUniquenessId());
